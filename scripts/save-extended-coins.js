...
async function saveExtendedCoins() {
  ...
  try {
    ...
    // Расширенные параметры для получения большего количества монет
    const params = {
      totalLimit: 600,        // Анализируем топ-600 монет (в 3 раза больше)
      minMarketCapBTC: 20,    // Снижаем минимальную капитализацию до 20 BTC
      currency: 'btc',        // В BTC парах
      maxResults: 300         // Показываем топ-300 из отфильтрованных (в 3 раза больше)
    };
    ...
    console.log(`📈 Увеличение количества монет в ${finalCoins.length / 106} раз (было 106, стало ${finalCoins.length})`);
